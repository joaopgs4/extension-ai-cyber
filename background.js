...
// Global state object storing requests, cookies, and messages from content scripts
const state = {
  requests: [],          // Array of {id, url, method, tabId, initiator, thirdParty, time, statusCode, setCookieCount, dangerous, blocked}
  cookies: [],           // Array of captured cookies
  pageMessages: []       // Messages sent from page scripts (localStorage, cookie writes)
};

// EasyList variables
let easyListRules = [];  // Compiled regex rules
let easyListRaw = '';    // Raw EasyList text
const EASYLIST_URL = 'https://easylist.to/easylist/easylist.txt'; // URL to fetch EasyList
const EASYLIST_REFRESH_MS = 1000 * 60 * 60 * 6; // Refresh every 6 hours
...
/**
 * Checks if a request is third-party relative to its tab.
...
async function isThirdParty(requestUrl, tabId){
  ...
  try { 
    ...
    return new URL(tab.url).hostname !== new URL(requestUrl).hostname; 
  } catch(e){ 
    return true; 
  }
}

// --- EasyList loader and parser ---
...
/**
 * Converts an EasyList rule line into a RegExp.
 * Ignores comments, exceptions, or cosmetic rules.
 * @param {string} rule
 * @returns {RegExp|null}
 */
function ruleToRegex(rule){
  rule = rule.trim(); 
  if (!rule || rule.startsWith('!') || rule.startsWith('@@') || rule.includes('##') || rule.includes('#@#')) 
    return null;
  ...
  for(let i=0;i<rule.length;i++){ 
    const ch=rule[i]; 
    if(ch==='*'){regexStr+='.*'; continue;} 
    if(ch==='^'){regexStr+='[^A-Za-z0-9_\\-.%]'; continue;} 
    regexStr+=escapeRegex(ch); 
  }

  if(prefixRegex) regexStr=prefixRegex+regexStr; 
  if(anchoredStart) regexStr='^'+regexStr; 
  if(anchoredEnd) regexStr+= '$';

  try { return new RegExp(regexStr); } catch(e){ return null; }
}

/** Fetches EasyList, parses it, and updates easyListRules */
async function fetchEasyList(){
  try {
    const resp=await fetch(EASYLIST_URL); 
    if(!resp.ok) throw new Error('EasyList fetch failed '+resp.status);
    const text=await resp.text(); 
    if(text===easyListRaw) return; 
    easyListRaw=text;

    const lines=text.split(/\r?\n/); 
    easyListRules=[]; 
    for(const L of lines){ 
      const r=ruleToRegex(L); 
      if(r) easyListRules.push(r); 
    }
    console.log('EasyList loaded. Rules:', easyListRules.length);
  } catch(e){ 
    console.warn('Failed to load EasyList:',e); 
  }
}
fetchEasyList(); 
setInterval(fetchEasyList,EASYLIST_REFRESH_MS);

/** Returns true if a URL matches EasyList rules */
function matchesEasyList(url){ 
  if(!easyListRules||easyListRules.length===0) return false; 
  try{ 
    for(const re of easyListRules) if(re.test(url)) return true; 
  } catch(e){} 
  return false; 
}
...
/** Captures and optionally blocks network requests */
browser.webRequest.onBeforeRequest.addListener(
  details => {
    ...
    isThirdParty(details.url,details.tabId).then(tp=>{
      const r=state.requests.find(x=>x.id===details.requestId); 
      if(r) r.thirdParty=tp; 
    }).catch(()=>{});

    if(isDangerous) return { cancel:true }; 
    return {};
  },
  { urls:["<all_urls>"] },
  ["blocking"]
...
/** Captures completed requests and response headers, including cookies */
  ...
  details=>{
    ...
    if(r){ 
      ...
      if(details.responseHeaders){ 
        const sc=details.responseHeaders.filter(h=>h.name.toLowerCase()==='set-cookie').length; 
        r.setCookieCount=sc;
        if(sc>0) state.cookies.push({
          time:Date.now(),
          removed:false,
          cause:"set-cookie-header",
          cookie:`from-response to ${details.url} (${sc})`,
          source:"response",
...
browser.cookies.onChanged.addListener(change=>{
  state.cookies.push({
    ...
    cookie:{
      ...
      path:change.cookie.path,
      storeId:change.cookie.storeId,
      secure:change.cookie.secure,
      httpOnly:change.cookie.httpOnly,
      sameSite:change.cookie.sameSite
    },
    source:"browser-cookie-api"
  });
});
...
browser.runtime.onMessage.addListener((msg,sender)=>{
  if(msg && msg.__api_mapper){
    ...
    payload.tabId=sender.tab?sender.tab.id:null; 
    payload.time=Date.now();
    state.pageMessages.push(payload);
    if(payload.type==='cookie-set-script'){ 
      state.cookies.push({
        time:Date.now(),
        removed:false,
        cause:"document.cookie",
...
/** Checks if a URL matches a manual blocked entry */
...
// Updated matchesEasyList to include manual blocked URLs
function matchesEasyList(url){
...
// --- Messages to add/remove manual blocked URLs ---
browser.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (!msg) return;
